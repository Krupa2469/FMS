...
async function shareRTIWhatsApp(){
    if(window.FMSWhatsAppService?.share){
        await window.FMSWhatsAppService.share({module:"RTI",title:"RTI Application Report",fields:[
            {id:"applicationNumber",label:"Application Number"},{id:"applicationDate",label:"Application Date"},{id:"dueDate",label:"Due Date"},{id:"applicantName",label:"Applicant Name"},{id:"mobileNumber",label:"Mobile Number"},{id:"district",label:"District"},{id:"mandal",label:"Mandal"},{id:"village",label:"Village"},{id:"officeFileNo",label:"File No."},{id:"officeSubject",label:"Subject"},{id:"officeStatus",label:"Status of File"}
        ],element:document.querySelector(".form-container"),message:(m)=>showRTIMessage(m,"info")});
        return;
    }
    const text=`RTI Application ${val("applicationNumber")}\nApplicant: ${val("applicantName")}\nFile No.: ${val("officeFileNo")}\nStatus: ${val("officeStatus")}`;
