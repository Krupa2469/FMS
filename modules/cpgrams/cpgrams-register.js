...
function registerEvents() {
    const btnWhatsAppDaily = document.getElementById("btnWhatsAppDaily");
    if (btnWhatsAppDaily) btnWhatsAppDaily.addEventListener("click", async () => {
        if (window.FMSWhatsAppService) {
            await window.FMSWhatsAppService.shareDailyStatus({
                module: "CPGRAMS",
                title: "CPGRAMS Daily Status Report",
                element: document.querySelector(".table-responsive") || document.querySelector("main") || document.body
            });
        }
    });
