"use strict";
/* ============================================================
   GRIEVANCES REGISTER CONTROLLER - v1.5.8
...
(function(window,document){
  ...
  function columnsForType(){
    ...
    if(k==="cpgrams")return [["registrationNo","Registration No."],["dateReceived","Date Received"],["dueDate","Due Date"],["daysStatus","Days Left / Overdue Days"],["complainantName","Complainant Name"],["district","District"],["subject","Subject"],["memoStatus","Memo / Letter Status"],["atrStatusView","ATR Status"],["finalStatusView","Final Status"]];
  ...
  function openRecord(id,mode){const r=allRecords.find(x=>x.id===id);if(!r)return;sessionStorage.setItem("selectedGrievance",JSON.stringify(r));location.href=`cpgrams.html?mode=${encodeURIComponent(mode)}&fullscreenForm=1&id=${encodeURIComponent(id)}&grievanceType=${encodeURIComponent(selectedType())}`;}
  async function deleteRecord(id){if(!confirm("Delete this record?"))return;try{const result=window.FMSCrud?await window.FMSCrud.softDelete(COLLECTION,id):null;if(!result){const db=await waitForDb();await db.collection(COLLECTION).doc(id).set({active:false,deletedOn:firebase.firestore.FieldValue.serverTimestamp(),updatedOn:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});}else if(!result.success){throw new Error(result.message||"Delete failed.");}await loadRegister();}catch(e){alert("Unable to delete record: "+(e.message||e));}}
  ...
  function bindExternalRefresh(){
    ...
    window.addEventListener("fmsRecordChanged",event=>{if(!event?.detail?.module||event.detail.module==="cpgrams")reload();});
  ...
  function init(){console.log("CPGRAMS Register v1.5.8 loaded");populateTypeDropdown();populateFY();bind();bindExternalRefresh();renderTable();loadRegister();}
