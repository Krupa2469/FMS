...
async function duplicateExists(data,excludeId){db=window.FMSCrud?await window.FMSCrud.waitForDb():db;const snap=await db.collection(currentType).get();const key=uniqueKeyFor(currentType,data);return snap.docs.some(d=>d.id!==excludeId&&uniqueKeyFor(currentType,d.data()||{})===key&&((d.data()||{}).active!==false));}
...
async function updateRecord(){if(!selectedId)return alert("Select a record to update.");try{db=window.FMSCrud?await window.FMSCrud.waitForDb():db;const data=collect();if(await duplicateExists(data,selectedId))throw new Error("A matching master record already exists.");const result=window.FMSCrud?await window.FMSCrud.update(currentType,selectedId,data):await db.collection(currentType).doc(selectedId).update(data).then(()=>({success:true})).catch(e=>({success:false,message:e.message||String(e)}));if(!result.success)throw new Error(result.message||"Unable to update record.");broadcastMasterDataUpdated(currentType);clearForm();await renderForm();await loadGrid();status("Record updated successfully.","success");}catch(e){status(e.message||String(e),"danger");alert(e.message||e);}}
async function deleteById(id){if(!confirm("Delete this master record?"))return;try{db=window.FMSCrud?await window.FMSCrud.waitForDb():db;const result=window.FMSCrud?await window.FMSCrud.softDelete(currentType,id):await db.collection(currentType).doc(id).update({active:false,deletedOn:firebase.firestore.FieldValue.serverTimestamp(),modifiedOn:firebase.firestore.FieldValue.serverTimestamp()}).then(()=>({success:true})).catch(e=>({success:false,message:e.message||String(e)}));if(!result.success)throw new Error(result.message||"Unable to delete record.");broadcastMasterDataUpdated(currentType);if(selectedId===id)clearForm();await renderForm();await loadGrid();status("Record deleted successfully.","success");}catch(e){status("Delete failed: "+(e.message||e),"danger");alert("Delete failed: "+(e.message||e));}}
...
async function start(){if(started)return;try{db=window.FMSCrud?await window.FMSCrud.waitForDb():window.db||window.fmsFirebase?.db;}catch(e){status("Waiting for Firebase...","warning");return;}if(!db){status("Waiting for Firebase...","warning");return;}started=true;initUI();await ensureBuiltInDefaults();await switchMaster(currentType);}
