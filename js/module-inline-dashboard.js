...
(function(){
  ...
  function card(label,value,filter,icon,theme){const href=`${cfg.register}?filter=${encodeURIComponent(filter)}&fy=${encodeURIComponent(currentFY())}`;return `<div class="col-12 col-sm-6 col-lg-4 col-xl-3"><a class="text-decoration-none text-reset" href="${href}"><div class="card h-100 shadow-sm border-0 fms-inline-metric-card"><div class="card-body text-center"><div class="fs-2 text-${theme}"><i class="bi ${icon}"></i></div><div class="text-muted small mt-2">${esc(label)}</div><div class="display-6 fw-bold text-${theme}">${esc(value)}</div><div class="small text-primary mt-2">View filtered data <i class="bi bi-arrow-right"></i></div></div></div></a></div>`;}
