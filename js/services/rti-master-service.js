...
(function(window){
    ...
    async function getMandals(district){
        ...
        if(database){
            try{
                const snap=await database.collection("mandals")
                    .where("district","==",district).get();
                snap.forEach(doc=>{
                    const d=doc.data()||{};
                    if(d.active!==false && d.name) values.push(clean(d.name));
                });
            }catch(e){ console.warn("RTI mandal master Firestore load failed:",e); }
        }
    ...
    async function getVillages(district,mandal){
        ...
        if(database){
            try{
                const snap=await database.collection("villages")
                    .where("district","==",district)
                    .where("mandal","==",mandal).get();
                snap.forEach(doc=>{
                    const d=doc.data()||{};
                    if(d.active!==false && d.name) values.push(clean(d.name));
                });
            }catch(e){ console.warn("RTI village master Firestore load failed:",e); }
        }
