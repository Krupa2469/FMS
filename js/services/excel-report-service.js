...
(function(window, document){
    ...
    async function exportAndWhatsApp(module, options={}){
        ...
        if(window.FMSWhatsAppService && typeof window.FMSWhatsAppService.shareDailyStatus==="function"){
            shareResult=await window.FMSWhatsAppService.shareDailyStatus({
                module,
                title:`${module} Daily Status Report`,
                element:options.element || document.querySelector(".report-container") || document.body,
                extraText:`Excel report exported: ${result.filename}\nFinancial Year: ${result.fy.label}`
            });
        }else if(window.open){
            window.open("https://wa.me/?text="+encodeURIComponent(`*${module} Daily Status Report*\nFinancial Year: ${result.fy.label}\nExcel report exported: ${result.filename}`),"_blank");
        }
        return {excel:result,share:shareResult};
    }
